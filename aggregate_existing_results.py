...
This script scans the results directory for individual integration_*.json files
...
def find_integration_result_files(results_dir: Path) -> List[Path]:
    """
    Find all individual integration result files in the results directory.
...
def main():
    ...
    if not file_paths:
        print("ℹ No individual integration result files found.")
        print("  Looking for files matching pattern: integration_*.json")
        print(f"  In directory: {results_dir}")
        return
    
    print(f"Found {len(file_paths)} individual integration result files:")
